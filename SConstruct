...
env.Program('client_test', client_test_src, LIBS=['rpcframe', 'pthread', 'uuid', 'protobuf'], LIBPATH = ['.', 'protobuf-261/lib/'], CPPPATH = ['./include', 'common/include', 'client/include'])

server_test_src = Split('test/server_test.cpp')
env.Program('server_test', server_test_src, LDFLAGS=[''], LIBS=['rpcframe', 'protobuf', 'pthread'], LIBPATH = ['.', 'protobuf-261/lib/'], CPPPATH = ['server/include', 'common/include'])
